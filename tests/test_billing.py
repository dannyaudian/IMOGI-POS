...
def test_start_cashier_shift_creates_session(billing_module):
    billing, frappe = billing_module

    frappe.has_role = lambda role: role == 'Cashier'

    created = {}

    class OpeningShiftDoc(types.SimpleNamespace):
        def insert(self, ignore_permissions=True):
            self.name = 'SHIFT-1'
            created['opening'] = self
            return self

    class SessionDoc(types.SimpleNamespace):
        def insert(self, ignore_permissions=True):
            self.name = 'SESSION-1'
            created['session'] = self
            return self

    def get_doc(args, name=None):
        if isinstance(args, dict):
            if args['doctype'] == 'POS Opening Shift':
                return OpeningShiftDoc(**args)
            if args['doctype'] == 'POS Session':
                return SessionDoc(**args)
        raise Exception('unexpected doctype')

    frappe.get_doc = get_doc
    billing.get_active_pos_session = lambda scope=None: None

    result = billing.start_cashier_shift('P1', 100)

    assert result['session_id'] == 'SESSION-1'
    assert result['start_time'] == frappe.utils.now_datetime()
    assert created['opening'].opening_balance == 100
    assert created['session'].opening_shift == 'SHIFT-1'


def test_start_cashier_shift_requires_cashier_role(billing_module):
    billing, frappe = billing_module
    frappe.has_role = lambda role: False

    with pytest.raises(frappe.PermissionError):
        billing.start_cashier_shift('P1', 100)


def test_start_cashier_shift_blocks_active_session(billing_module):
    billing, frappe = billing_module
    frappe.has_role = lambda role: True
    billing.get_active_pos_session = lambda scope=None: 'SESSION-1'
