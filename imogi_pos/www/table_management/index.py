...
def get_context(context):
    """Context builder for Table Management Editor page."""
    try:
        # Get branding info
        ...
        context.title = _("Table Management")
        
        # Add React bundle URLs and initial state (auto-loads from manifest.json)
        add_react_context(context, 'table-management-editor', {
            'branding': branding
        })
        ...
        frappe.log_error(f"Error in table_management get_context: {str(e)}")
        set_setup_error(context, "generic", str(e), page_name=_("Table Management"))
        context.title = _("Table Management")
        return context
