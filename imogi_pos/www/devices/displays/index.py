...
def get_context(context):
    ...
    This page supports both public customer view and authenticated admin mode.
    
    - Guest users see fullscreen customer display
    - Authenticated managers see admin controls + display preview
    """
    context.title = _("Customer Display")

    # Get user role context for conditional rendering
    context.user_context = get_user_role_context()
    context.is_admin_mode = context.user_context.get("is_manager", False)
    ...
    # Admin mode: Load additional context for device management
    if context.is_admin_mode:
        context.all_devices = get_all_display_devices()
        context.available_profiles = get_display_profiles()
    ...
    # Provide currency symbol to template
    default_currency = frappe.defaults.get_global_default("currency")
    context.currency_symbol = frappe.get_doc("Currency", default_currency).symbol
...
def get_all_display_devices():
    """Get all Customer Display Devices (admin only)."""
    if not frappe.db.exists("DocType", "Customer Display Device"):
        return []
    ...
    try:
        devices = frappe.get_all(
            "Customer Display Device",
            fields=["name", "device_id", "device_name", "status", "branch", "last_heartbeat"],
            order_by="device_name"
        )
        return devices
    except Exception as e:
        frappe.log_error(f"Error fetching display devices: {str(e)}")
        return []


def get_display_profiles():
    """Get all Customer Display Profiles (admin only)."""
    if not frappe.db.exists("DocType", "Customer Display Profile"):
        return []
    
    try:
        profiles = frappe.get_all(
            "Customer Display Profile",
            fields=["name", "profile_name", "layout"],
            order_by="profile_name"
        )
        return profiles
    except Exception as e:
        frappe.log_error(f"Error fetching display profiles: {str(e)}")
        return []
