...
(function() {
    ...
    // Check if we're in a Frappe Desk environment by looking at the page URL/path
    // Desk pages are under /app or /desk, while standalone pages are under custom routes
    const isDeskPage = window.location.pathname.startsWith('/app') || 
                       window.location.pathname.startsWith('/desk') ||
                       document.body.getAttribute('data-page-route')?.startsWith('app');
    
    // If Frappe Desk environment is detected, don't load polyfill at all
    // Frappe will load its own complete API via desk.bundle.js
    if (isDeskPage) {
        console.log('Frappe Desk page detected - polyfill skipped, native Frappe APIs will load');
        
        // Ensure __ translation function exists as a fallback
        if (typeof window.__ === 'undefined') {
            window.__ = function(txt) {
                if (arguments.length > 1 && Array.isArray(arguments[1])) {
                    // Handle format strings like __('Text {0}', [value])
                    let formatted = txt;
                    arguments[1].forEach((val, idx) => {
                        formatted = formatted.replace(new RegExp(`\\{${idx}\\}`, 'g'), val);
                    });
                    return formatted;
                }
                return txt;
    ...
    window.frappe.call = function(opts) {
        ...
        if (!method || method === 'undefined') {
            console.error('[frappe.call] Method is undefined or empty', opts);
            const error = new Error('Method parameter is required for frappe.call');
            if (errorCallback) {
                errorCallback(error);
            }
    ...
    console.log('Frappe polyfill loaded for standalone page at:', window.location.pathname);
})();
